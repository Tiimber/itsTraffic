...
public class WayObjectStraight {
	...
	public static void create (long key, List<WayReference> wayReferences, string materialId) {
		...
		if (wayReferences.Count == 2 && wayBounds [0].Intersects (wayBounds [1])) {
			// Only two ways and they intersect, special logic
			meshPoints = getMeshPointsForComplexTwoWay(wayReferences, wayBounds, pos);
		} else {
			meshPoints = getMeshPointsForNonComplex(wayReferences, wayBounds, pos);
		}
...
//		Debug.Log ("Intersection");
	...
	static List<Vector3> getMeshPointsForComplexTwoWay (List<WayReference> wayReferences, List<Bounds> wayBounds, Pos pos)
	{
		...
		// Add bezier points between the ways, from "right" point in way2 to "left" pount in way1
